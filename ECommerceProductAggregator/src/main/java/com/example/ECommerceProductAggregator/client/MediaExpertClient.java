...
@Component
@Slf4j
public class MediaExpertClient {
    private final RestClient restClient;

    public MediaExpertClient(@Qualifier("mediaExpert") RestClient restClient) {
        this.restClient = restClient;
    }

    @Cacheable(
            cacheNames = "offers",
            key = "#productName + 'mediaExpert'",
    ...
    public List<ExternalApiResponse> getProduct(String productName) {
        return restClient.get()
                ...
                .onStatus(HttpStatusCode::isError, (request, response) -> {
                    log.error("Error retrieving MediaExpert product: {}", productName);
