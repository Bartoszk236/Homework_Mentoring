...
class MediaExpertClientTest {
    private static final String BASE_URL = "http://localhost:8080/MediaExpert";
    ...
    void setUp() {
        ...
        mediaExpertClient = new MediaExpertClient(restClient);
        productName = "Xbox";
    }
    ...
    void givenValidResponseWhenGetProductThenReturnProduct() {
        ...
        mockServer.expect(requestTo(BASE_URL + "/" + productName))
                ...
                .andRespond(withSuccess(ExternalApiResponses.getValidResponseFromMediaExpert(), MediaType.APPLICATION_JSON));

        //when
        List<ExternalApiResponse> result = mediaExpertClient.getProduct(productName);

        //then
        assertThat(result).hasSize(1);
        assertThat(result.getFirst().name()).isEqualTo(productName);
        mockServer.verify();
    }
    ...
    void givenErrorResponseWhenGetProductThenThrowProductNotFoundException() {
        ...
        //when / then
        assertThatThrownBy(() -> mediaExpertClient.getProduct(productName))
