...
public class ControllerV2 {
    ...
    public ResponseEntity<Object> getUsersByURL() {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUsers(ApiVersion.V2));
    }

    @GetMapping(value = "/api/users", headers = "API-Version=2.0")
    public ResponseEntity<Object> getUsersByHeaders() {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUsers(ApiVersion.V2));
    }

    @GetMapping(value = "/api/users", params = "version=2.0")
    ...
    public ResponseEntity<Object> getUserByURL(@PathVariable Long id) {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUser(ApiVersion.V2, id));
    }

    @GetMapping(value = "/api/users/{id}", headers = "API-Version=2.0")
    public ResponseEntity<Object> getUserByHeader(@PathVariable Long id) {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUser(ApiVersion.V2, id));
    }

    @GetMapping(value = "/api/users/{id}", params = "version=2.0")
    public ResponseEntity<Object> getUserByParams(@PathVariable Long id) {
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUser(ApiVersion.V2, id));
    }

    @DeleteMapping(value = "/api/v2/users/{id}")
    public ResponseEntity<HttpStatus> deleteUserByURL(@PathVariable Long id) {
        userService.deleteUser(id);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @DeleteMapping(value = "/api/users{id}", headers = "API-Version=2.0")
    public ResponseEntity<HttpStatus> deleteUserByHeader(@PathVariable Long id) {
        userService.deleteUser(id);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @DeleteMapping(value = "/api/users{id}", params = "version=2.0")
    public ResponseEntity<HttpStatus> deleteUserByParam(@PathVariable Long id) {
        userService.deleteUser(id);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @PutMapping(value = "/api/v2/users/{id}")
    public ResponseEntity<Object> updateUserByURL(@PathVariable Long id, @RequestBody UserRequest userRequest) {
        userService.updateUser(ApiVersion.V2, id, userRequest);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @PutMapping(value = "/api/users/{id}", headers = "API-Version=2.0")
    public ResponseEntity<Object> updateUserByHeader(@PathVariable Long id, @RequestBody UserRequest userRequest) {
        userService.updateUser(ApiVersion.V2, id, userRequest);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @PutMapping(value = "/api/users/{id}", params = "version=2.0")
    ...
    @PostMapping(value = "/api/users", headers = "API-Version=2.0")
    public ResponseEntity<HttpStatus> createUserByHeader(@RequestBody UserRequest userRequest) {
        userService.createUser(userRequest);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @PostMapping(value = "/api/users", params = "version=2.0")
